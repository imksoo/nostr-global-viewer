...
export async function speakNote(event: nostr.Event, profile: Profile, vol: string, waitTime: number = 1500) {
  ...
  setTimeout(() => {
    ...
    utterUserNameText = utterUserNameText
      ...
      .replace(/(nostr:)?(nprofile|nrelay|nevent|naddr|nsec|npub|note)1\S*/g, "")
      .replace("αυγοτάραχο σολωμου", "いくらどん");
    display_name += "さん";

    const utterUserName = new SpeechSynthesisUtterance(utterUserNameText);
    utterUserName.lang = "ja-JP";
    utterUserName.volume = volume;
    console.log("Speech Username: ", utterUserNameText);
    ...
    console.log("Speech Content: ", utterEventContent);
